...
/**
 * 函数：cJSON_ParseWithOpts
 * 
 * 【作用】
 * JSON解析的核心入口，支持错误定位和格式检查
 * 
 * 【设计思想】
 * 1. 分层设计：cJSON_Parse → cJSON_ParseWithOpts → cJSON_ParseWithLengthOpts
 *    越往内层功能越强大，外层提供简化接口
 * 2. 防御性编程：立即检查NULL输入
 * 3. 自动计算长度：对用户隐藏细节，自动处理字符串终止符
 * 
 * 【参数说明】
 * @param value: JSON字符串
 * @param return_parse_end: [输出] 解析结束位置（错误定位用）
 * @param require_null_terminated: 是否必须遇到'\0'才结束
 * 
 * 【返回值】
 * cJSON树根节点，失败返回NULL
 * 
...
CJSON_PUBLIC(cJSON*) cJSON_ParseWithOpts(const char* value, const char** return_parse_end, cJSON_bool require_null_terminated)
{
    ...
    buffer_length = strlen(value) + sizeof("");  /* +1 包含终止符 '\0' */
 ...
 * 
 * 【作用】
 * 将 JSON 字符串解析为 cJSON 树结构（简化版接口）
 * 
 * 【实现方式】
 * 这是一个包装函数，实际工作由 cJSON_ParseWithOpts 完成
 * 
 * 【参数】
 ...
 * 
 * 【使用示例】
 * cJSON *root = cJSON_Parse("{\"name\":\"cJSON\"}");
 * if (root == NULL) {
 *     printf("解析失败\n");
 * }
 * 
 * 【内存管理】
 * 返回的 cJSON 树需要调用者用 cJSON_Delete(root) 释放
 * 
 * 【设计思路】
 * 提供最简洁的 API 接口，将复杂参数用默认值(0,0)隐藏，
 * 符合"简单接口易用，复杂接口可用"的设计哲学
 */
